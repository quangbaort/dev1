<template>
  <div>
    ...
    <v-expand-transition>
      <v-autocomplete v-show="shallShowFullSearchLocal"
        ref="refAutocomplete" :search-input.sync="searchQueryLocal"
        :items="searchQueryLocal ? data : []" hide-no-data
        no-data-text="" item-text="title" item-value="title"
        class="app-bar-autocomplete-box" return-object
        :color="$vuetify.theme.dark ? 'white' : null" rounded
        attach  hide-details  :filter="filter"  auto-select-first
        :menu-props="{ maxHeight: 500, transition: 'slide-y-transition', contentClass: 'list-style' }"
        @input="valueSelected" @keyup.esc="shallShowFullSearchLocal = false"
